// colors

export const baseFontColor = "rgb(255, 255, 255)";
export const black = "#323232";


export const gray = "#7d7d7d";
export const grayMedium = "#c4c4c4";
export const grayDark = "#323232";
export const grayLight = "#f1f5f5";
export const overlayColor = "rgba(199, 207, 207, 0.8)";
export const rose = "#c22d74";

export const white = "#fff";
export const tabelGray = "#eaeaea";

export const green = "#9cdd24";
export const greenDark = "#7cb01c";
export const greenLight = "#b9e765";
export const greenTransparent = "rgba(175, 227, 79, 0.8)";

export const purple = "rgb(207, 116, 223)";
export const purpleDark = "#9224A6";
export const purpleOverlay = "#5A1666";
export const purpleOverlayDark = "#2D0B33";


// theme colors
export const theme = {
  activeMenuOption: greenDark,
  autofill: purpleDark,
  autofillSelected: purpleOverlayDark,
  baseFont: baseFontColor,
  dark: purpleOverlayDark,
  disabled: gray,
  divider: grayLight,
  dividerDark: grayMedium,
  error: rose,
  hoverLightBackground: greenLight,
  light: grayLight,
  lightFont: white,
  overlay: overlayColor,
  primary: green,
  primaryDark: greenDark,
  primaryLight: greenLight,
  primaryTransparent: greenTransparent,
  secondary: purple,
  secondaryDark: purpleDark,
  secondaryOverlay: purpleOverlay,
  secondaryOverlayDark: purpleOverlayDark,
  success: green,
  tableDivider: tabelGray,
  white,
};

// typography
export const baseFontFamily = "'Inter', sans-serif";
export const baseFontSize = "1rem"; // 16px
export const baseLineHeight = "1.25rem"; // 20px
export const boldFontWeight = 600;
export const extraBoldFontWeight = 800;
export const h1FontSize = "4rem"; // 64px
export const h1LineHeight = 1;
export const h3FontSize = "1.5rem"; // 24px
export const h4FontSize = "1.125rem"; // 18px
export const labelFontSize = "0.75rem"; // 12px
export const smallFontSize = "0.875rem"; // 14px

// spacing
export const spacer = 1; // rem

// breakpoints
export const mediumScreen = 992;
export const smallScreen = 540;
