...
import { KaavishAPI } from "../index";
import { RequireAtLeastOne } from "../tsHelpers";
import { useAuth, useKaavishClient } from "./helpers";
import {
...
type OmittedOptions<T extends keyof KaavishAPI> = Omit<
  Options<T>,
  "onUpdate" | "onComplete" | "onError"
> & { skip?: boolean };
type AdditionalAPI = ReturnType<KaavishAPI["watchQuery"]>;
type Result<TData> = {
  data: TData | null;
  loading: boolean;
...
>(
  ...
  const {
    ...
    () =>
      (Kaavish[query] as AdditionalAPI)(variables, {
        ...(options as any),
...
export const queryWithVariablesFactory = <T extends keyof KaavishAPI>(
  query: T
) => (variables: Variables<T>, options?: OmittedOptions<T>) =>
  useQuery(query, variables, options);

export const queryFactory = <T extends keyof KaavishAPI>(query: T) => (
