...
from kaavish.product.models import Attribute, AttributeValue
from kaavish.product.utils.attributes import associate_attribute_values_to_instance
from kaavish.seo.schema.email import (
    get_order_confirmation_markup,
    get_organization,
    get_product_data,
)
from kaavish.seo.schema.product import get_brand_from_attributes


def test_get_organization(site_settings):
    example_name = "kaavish Brand Name"
...
def test_get_brand_from_attributes(product):
    ...
    brands = AttributeValue.objects.bulk_create(
        [
            AttributeValue(attribute=attribute, name="kaavish", slug="kaavish"),
    ...
    assert brand_names_str == "kaavish, Mirumee"
