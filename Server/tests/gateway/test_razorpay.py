...
from kaavish.payment.gateways.razorpay import (
    capture,
...
from kaavish.payment.gateways.razorpay.forms import (
    RazorPayCheckoutWidget,
    RazorPaymentForm,
)
from kaavish.payment.interface import GatewayConfig
...
def test_checkout_widget_render_without_prefill(razorpay_payment, gateway_config):
    ...
    assert widget.render() == (
        ...
        'data-image="image.png" data-key="public" data-name="kaavish" '
...
def test_get_plugin_configuration_hides_all_razor_secrets(settings):
    settings.PLUGINS = ["kaavish.payment.gateways.razorpay.plugin.RazorpayGatewayPlugin"]
