...
from kaavish.checkout import forms, utils
from kaavish.checkout.context_processors import checkout_counter
...
from kaavish.checkout.views import clear_checkout, update_checkout_line
from kaavish.core.exceptions import InsufficientStock
from kaavish.discount.models import Sale
from kaavish.product.models import Category
...
def test_get_or_create_checkout_from_request(
    ...
    monkeypatch.setattr("kaavish.checkout.utils.get_user_checkout", mock_get_for_user)
    monkeypatch.setattr(
        "kaavish.checkout.utils.get_or_create_anonymous_checkout_from_token",
...
def test_get_checkout_from_request(
    ...
    mock_get_for_anonymous = Mock(return_value=anonymous_checkout)
    monkeypatch.setattr(
        "kaavish.checkout.utils.get_anonymous_checkout_from_token",
        mock_get_for_anonymous,
    )
    request = checkout_request_factory(user=None, token=token)
    returned_checkout = utils.get_checkout_from_request(request, queryset)
...
def test_checkout_counter(monkeypatch):
    monkeypatch.setattr(
        "kaavish.checkout.context_processors.get_checkout_from_request",
...
def test_check_product_availability_and_warn(monkeypatch, checkout, product):
    ...
    monkeypatch.setattr(
        "kaavish.checkout.utils.contains_unavailable_variants", Mock(return_value=False)
    )
    ...
    monkeypatch.setattr(
        "kaavish.checkout.utils.contains_unavailable_variants", Mock(return_value=True)
    )
    monkeypatch.setattr(
        "kaavish.checkout.utils.remove_unavailable_variants",
...
def test_replace_checkout_line_form_when_insufficient_stock(
    ...
    monkeypatch.setattr(
        "kaavish.product.models.ProductVariant.check_quantity",
...
def test_view_update_checkout_quantity(client, request_checkout_with_item, monkeypatch):
    monkeypatch.setattr(
        "kaavish.checkout.views.to_local_currency", lambda price, currency: price
    )
