...
from kaavish.graphql.product.types import Product
from kaavish.graphql.views import handled_errors_logger, unhandled_errors_logger
...
def test_invalid_query_graphql_errors_are_logged_in_another_logger(
    ...
    assert graphql_log_handler.messages == [
        "kaavish.graphql.errors.handled[ERROR].GraphQLError"
    ]
...
def test_invalid_syntax_graphql_errors_are_logged_in_another_logger(
    ...
    assert response.status_code == 400
    assert graphql_log_handler.messages == [
        "kaavish.graphql.errors.handled[ERROR].GraphQLSyntaxError"
    ]
...
def test_permission_denied_query_graphql_errors_are_logged_in_another_logger(
    ...
    assert response.status_code == 200
    assert graphql_log_handler.messages == [
        "kaavish.graphql.errors.handled[ERROR].PermissionDenied"
    ]
...
def test_unexpected_exceptions_are_logged_in_their_own_logger(
    ...
    assert response.status_code == 200
    assert graphql_log_handler.messages == [
        "kaavish.graphql.errors.unhandled[ERROR].NotImplementedError"
    ]
