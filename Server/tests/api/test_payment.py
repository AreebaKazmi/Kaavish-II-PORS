...
from kaavish.core.utils import get_country_name_by_code
from kaavish.graphql.payment.enums import OrderAction, PaymentChargeStatusEnum
from kaavish.payment.interface import CreditCardInfo, CustomerSource, TokenConfig
from kaavish.payment.models import ChargeStatus, Payment, TransactionKind
from kaavish.payment.utils import fetch_customer_id, store_customer_id
...
def test_payment_void_gateway_error(
    ...
    payment_id = graphene.Node.to_global_id("Payment", payment_txn_preauth.pk)
    variables = {"paymentId": payment_id}
    monkeypatch.setattr("kaavish.payment.gateways.dummy.dummy_success", lambda: False)
    response = staff_api_client.post_graphql(
        VOID_QUERY, variables, permissions=[permission_manage_orders]
    )
    content = get_graphql_content(response)
...
def test_list_payment_sources(
    ...
    mock_get_source_list = mocker.patch(
        "kaavish.graphql.account.resolvers.gateway.list_payment_sources",
        return_value=[source],
        autospec=True,
    )
