...
from kaavish.account.utils import create_superuser
from kaavish.core.storages import S3MediaStorage
from kaavish.core.utils import (
    Country,
    build_absolute_uri,
    create_thumbnails,
    format_money,
...
from kaavish.core.utils.text import get_cleaner, strip_html
from kaavish.core.weight import WeightUnits, convert_weight
from kaavish.discount.models import Sale, Voucher
from kaavish.giftcard.models import GiftCard
from kaavish.order.models import Order
from kaavish.product.models import ProductImage
...
def test_get_country_by_ip(ip_data, expected_country, monkeypatch):
    monkeypatch.setattr("kaavish.core.utils.georeader.get", Mock(return_value=ip_data))
...
def test_create_fake_order(db, monkeypatch, image, media_root):
    ...
    monkeypatch.setattr(
        "kaavish.core.utils.random_data.get_image", Mock(return_value=image)
    )
    for _ in random_data.create_shipping_zones():
        pass
