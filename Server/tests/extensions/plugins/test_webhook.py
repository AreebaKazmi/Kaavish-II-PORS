...
from kaavish.account.models import ServiceAccount
from kaavish.extensions.manager import get_extensions_manager
from kaavish.extensions.plugins.webhook import create_hmac_signature
from kaavish.extensions.plugins.webhook.tasks import trigger_webhooks_for_event
from kaavish.webhook import WebhookEventType
from kaavish.webhook.payloads import (
...
@pytest.mark.vcr
@mock.patch(
    "kaavish.extensions.plugins.webhook.tasks.requests.post", wraps=requests.post
)
def test_trigger_webhooks_for_event(
    ...
    expected_headers = {
        "X-kaavish-Event": "order_created",
        "X-kaavish-Domain": "mirumee.com",
    }
...
@mock.patch("kaavish.extensions.plugins.webhook.tasks.send_webhook_request.delay")
def test_trigger_webhooks_for_event_calls_expected_events(
    ...
    """Confirm that kaavish executes only valid and allowed webhook events."""
...
def test_trigger_webhooks_for_event_with_secret_key(
    ...
    expected_headers = {
        "X-kaavish-Event": "order_created",
        "X-kaavish-Domain": "mirumee.com",
        "X-kaavish-HMAC-SHA256": f"sha1={expected_signature}",
    }
...
@mock.patch("kaavish.extensions.plugins.webhook.plugin.trigger_webhooks_for_event.delay")
def test_order_created(mocked_webhook_trigger, settings, order_with_lines):
    settings.PLUGINS = ["kaavish.extensions.plugins.webhook.plugin.WebhookPlugin"]
