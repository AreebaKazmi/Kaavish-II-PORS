...
from kaavish.core.taxes import TaxError, quantize_price
from kaavish.extensions.manager import get_extensions_manager
from kaavish.extensions.models import PluginConfiguration
from kaavish.extensions.plugins.avatax import (
    AvataxConfiguration,
    checkout_needs_new_fetch,
    generate_request_data_from_checkout,
    get_cached_tax_codes_or_fetch,
)
from kaavish.extensions.plugins.avatax.plugin import AvataxPlugin
...
def test_calculate_checkout_line_total(
    ...
    settings.PLUGINS = ["kaavish.extensions.plugins.avatax.plugin.AvataxPlugin"]
    monkeypatch.setattr(
        "kaavish.extensions.plugins.avatax.plugin.get_cached_tax_codes_or_fetch",
...
def test_get_cached_tax_codes_or_fetch(monkeypatch, settings):
    monkeypatch.setattr("kaavish.extensions.plugins.avatax.cache.get", lambda x, y: {})
...
def test_checkout_needs_new_fetch(monkeypatch, checkout_with_item, address):
    monkeypatch.setattr("kaavish.extensions.plugins.avatax.cache.get", lambda x: None)
    checkout_with_item.shipping_address = address
    config = AvataxConfiguration(
        username_or_account="wrong_data", password_or_license="wrong_data"
    )
...
def test_order_created(settings, order, monkeypatch):
    ...
    mocked_task = Mock()
    monkeypatch.setattr(
        "kaavish.extensions.plugins.avatax.plugin.get_order_tax_data", Mock()
    )
    monkeypatch.setattr(
        "kaavish.extensions.plugins.avatax.plugin.api_post_request_task.delay",
        mocked_task,
    )
