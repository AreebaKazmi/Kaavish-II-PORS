...
from kaavish.core.taxes import quantize_price
from kaavish.dashboard.taxes.filters import get_country_choices_for_vat
from kaavish.extensions.manager import get_extensions_manager
from kaavish.extensions.plugins.vatlayer import (
...
from kaavish.extensions.plugins.vatlayer.plugin import VatlayerPlugin
...
@mock.patch("kaavish.dashboard.taxes.views.messages", create=True)
@mock.patch("kaavish.dashboard.taxes.views.call_command", create=True)
...
@mock.patch("kaavish.dashboard.taxes.views.messages", create=True)
@mock.patch("kaavish.dashboard.taxes.views.logger", create=True)
@mock.patch(
    "kaavish.dashboard.taxes.views.call_command",
...
def test_vatlayer_plugin_caches_taxes(vatlayer, monkeypatch, product, address):
    ...
    monkeypatch.setattr(
        "kaavish.extensions.plugins.vatlayer.plugin.get_taxes_for_country", mocked_taxes
    )
...
def test_calculate_checkout_total(
    ...
    manager = get_extensions_manager(
        plugins=["kaavish.extensions.plugins.vatlayer.plugin.VatlayerPlugin"]
    )
...
def test_get_plugin_configuration(vatlayer, settings):
    settings.PLUGINS = ["kaavish.extensions.plugins.vatlayer.plugin.VatlayerPlugin"]
    manager = get_extensions_manager()
...
def test_get_tax_rate_type_choices(vatlayer, settings, monkeypatch):
    ...
    monkeypatch.setattr(
        "kaavish.extensions.plugins.vatlayer.plugin.get_tax_rate_types",
