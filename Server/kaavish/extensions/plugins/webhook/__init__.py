...
def create_webhook_headers(
    ...
    headers = {"X-kaavish-Event": event_name, "X-kaavish-Domain": domain}
    if secret_key and body:
        kaavish_hmac_sha256 = signature_prefix + create_hmac_signature(
            body, secret_key, encoding
        )
        headers["X-kaavish-HMAC-SHA256"] = kaavish_hmac_sha256
    return headers
