...
print(f"len(triplet_in_shop_loader): {len(triplet_in_shop_loader)}")
...
# writer = SummaryWriter(log_dir=f"runs/{CATEGORIES}_categories/inshop={ENABLE_INSHOP_DATASET}"
#                                f"/lr=0.002/with_eastern")

writer = SummaryWriter(log_dir=f"runs/test/{CATEGORIES}_categories/inshop={ENABLE_INSHOP_DATASET}"
                               f"/lr=0.002/with_eastern")
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx and batch_idx % DUMP_INTERVAL == 0:
            print('Model saved to {}'.format(dump_model(model, epoch, batch_idx, with_eastern=True)))

    print('Model saved to {}'.format(dump_model(model, epoch)))


def test(step_no, full=False, topk=(1, 3, 5)):
    ...
    # correct = 0
    maxk = max(topk)
    correct_dict = {k: 0 for k in topk}
    ...
    for batch_idx, (data, target) in enumerate(test_loader):
        ...
        output = model(data)[0]  # FloatTensor of dim (test_batch_size, num_classes))
        ...
        # Calculate accuracy
        # pred = output.data.max(1, keepdim=True)[1]  # Tensor of dim (test_batch_size, 1). Contains indexes of max elems
        ...
        for k in topk:
            correct_dict[k] += correct_tensor[:k].view(-1).float().sum(0, keepdim=True)
    ...
    print('\nTest set: Average loss: {:.4f}'.format(float(test_loss)))
    writer.add_scalar('Loss/test', test_loss, step_no)

    top_k_str = ""
    for k in topk:
        top_k_str += "Top {} Accuracy: {}/{} ({:.0f}%)\n".format(
            k,
            int(correct_dict[k].item()),
            (test_batch_count * TEST_BATCH_SIZE),
            float(100. * correct_dict[k] / (test_batch_count * TEST_BATCH_SIZE))
        )
        writer.add_scalar(f"Accuracy/test/top-{k}",
                          float(100. * correct_dict[k] / (test_batch_count * TEST_BATCH_SIZE)),
                          step_no)
    print(top_k_str, "")
    # print(top_k_str)

    # inshop_top_n = [1, 10, 20, 50]
    # results = []
    # print("Testing in-shop retrieval")
    # for n in inshop_top_n:
    #     results.append(eval(retrieval_top_n=n))
    # inshop_acc_dict = eval()
    # writer.add_scalar('Accuracy/test/in-shop/top-10', inshop_acc_dict[10], step_no)
